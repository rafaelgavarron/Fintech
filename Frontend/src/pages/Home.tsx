...
function Home() {
    ...
    const [filterByBankAccount, setFilterByBankAccount] = useState<string>('');
  ...
  // Função para buscar o saldo total e os totais do mês
        ...
        try {
            ...
            expenses.forEach(expense => {
                ...
                if (expense.category && expense.category.trim() !== '') {
                    ...
                    categoryTotals[cat] = (categoryTotals[cat] || 0) + expense.expenseAmount;
                }
            });
        ...
        try {
            ...
            incomes.forEach(income => {
                ...
                if (income.category && income.category.trim() !== '') {
                    ...
                    categoryTotals[cat] = (categoryTotals[cat] || 0) + income.incomeAmount;
                }
            });
    ...
    const loadTimeSeriesData = useCallback(async () => {
        ...
        try {
            ...
            if (filterByCategory) {
                filteredExpenses = expenses.filter(e => e.category === filterByCategory);
                filteredIncomes = incomes.filter(i => i.category === filterByCategory);
            }
            ...
            filteredExpenses.forEach(expense => {
                const date = new Date(expense.expenseDate * 1000).toISOString().split('T')[0];
                if (!dataByDate[date]) {
            ...
            filteredIncomes.forEach(income => {
                const date = new Date(income.incomeDate * 1000).toISOString().split('T')[0];
                if (!dataByDate[date]) {
                    dataByDate[date] = { expenses: 0, incomes: 0 };
                }
    ...
    }, [currentOrganization, loadFinancialData, loadGoals, loadExpensesByCategory, loadIncomesByCategory]);
    ...
    }, [currentOrganization, filterByCategory, filterByDateRange, filterByBankAccount, loadTimeSeriesData]);
    ...
    return (
        ...
        <div className="container-fluid">
            <div className="alert alert-info">Carregando dados da organização...</div>
        </div>
        </div>
    );
    }
    ...
    <div className="main-content">
        <div className="container-fluid">
        <div className="mb-4">
            <h2 className="h2 fw-bold text-light mb-1">Olá, {user.name.split(' ')[0]}!</h2>
            <p className="text-muted mb-0">Bem-vindo de volta ao seu painel financeiro.</p>
        </div>
        ...
        <div className="row g-4 mb-5">
            ...
            <div className="col-lg-3 col-md-6">
                <div className="card h-100 shadow-sm" style={{ borderLeft: '4px solid #007bff', borderRadius: '8px' }}>
                    <div className="card-body p-4">
                        <div className="d-flex align-items-center justify-content-between mb-3">
                            <div className="p-2 rounded" style={{ backgroundColor: 'rgba(0, 123, 255, 0.1)' }}>
                                <i className="bi bi-wallet-fill text-primary fs-4"></i>
                            </div>
                        </div>
                        <h6 className="text-muted mb-2 small text-uppercase fw-semibold">Saldo Atual</h6>
                        {loading ? (
                            <p className="text-muted mb-0">Carregando...</p>
                        ) : (
                            <>
                                <h3 className={`fw-bold mb-2 ${saldoAtual >= 0 ? 'text-success' : 'text-danger'}`} style={{ fontSize: '1.8rem' }}>
                                    {formatCurrency(saldoAtual)}
                                </h3>
                                <small className="text-muted">Atualizado em {dataAtualizacao || 'N/A'}</small>
                            </>
                        )}
                    </div>
                </div>
            ...
            {/* Receitas (Mês) */}
            <div className="col-lg-3 col-md-6">
                <div className="card h-100 shadow-sm" style={{ borderLeft: '4px solid #28a745', borderRadius: '8px' }}>
                    <div className="card-body p-4">
                        <div className="d-flex align-items-center justify-content-between mb-3">
                            <div className="p-2 rounded" style={{ backgroundColor: 'rgba(40, 167, 69, 0.1)' }}>
                                <i className="bi bi-graph-up-arrow text-success fs-4"></i>
                            </div>
                        </div>
                        <h6 className="text-muted mb-2 small text-uppercase fw-semibold">Receitas (Mês)</h6>
                        {loading ? (
                            <p className="text-muted mb-0">Carregando...</p>
                        ) : (
            ...
            {/* Despesas (Mês) */}
            <div className="col-lg-3 col-md-6">
                <div className="card h-100 shadow-sm" style={{ borderLeft: '4px solid #dc3545', borderRadius: '8px' }}>
                    <div className="card-body p-4">
                        <div className="d-flex align-items-center justify-content-between mb-3">
                            <div className="p-2 rounded" style={{ backgroundColor: 'rgba(220, 53, 69, 0.1)' }}>
                                <i className="bi bi-graph-down-arrow text-danger fs-4"></i>
                            </div>
                        </div>
                        <h6 className="text-muted mb-2 small text-uppercase fw-semibold">Despesas (Mês)</h6>
                        {loading ? (
                            <p className="text-muted mb-0">Carregando...</p>
                        ) : (
            ...
            {/* Saldo Líquido */}
            <div className="col-lg-3 col-md-6">
                <div className="card h-100 shadow-sm" style={{ borderLeft: '4px solid #6f42c1', borderRadius: '8px' }}>
                    <div className="card-body p-4">
                        <div className="d-flex align-items-center justify-content-between mb-3">
                            <div className="p-2 rounded" style={{ backgroundColor: 'rgba(111, 66, 193, 0.1)' }}>
                                <i className="bi bi-piggy-bank-fill text-info fs-4"></i>
                            </div>
                        </div>
                        <h6 className="text-muted mb-2 small text-uppercase fw-semibold">Saldo Líquido</h6>
                        {loading ? (
                            <p className="text-muted mb-0">Carregando...</p>
                        ) : (
                            <h3 className={`fw-bold mb-0 ${(totalReceitasMes - totalDespesasMes) >= 0 ? 'text-success' : 'text-danger'}`} style={{ fontSize: '1.8rem' }}>
                ...
                <div className="card h-100 shadow-sm" style={{ borderRadius: '12px' }}>
                    <div className="card-body p-4">
                        ...
                        <div className="d-grid gap-2">
                            ...
                            >
                                <i className="bi bi-plus-circle-fill me-2"></i> Adicionar Receita
                            </Link>
                            <Link
                                to="/despesas/novo"
                                ...
                                style={{ borderRadius: '6px' }}
                            >
                                <i className="bi bi-dash-circle-fill me-2"></i> Adicionar Despesa
                            </Link>
                            <Link
                                ...
                                style={{ borderRadius: '6px' }}
                            >
                                <i className="bi bi-graph-up me-2"></i> Adicionar Investimento
                            </Link>
                            <Link
                                to="/metas/novo"
                ...
                <div className="card h-100 shadow-sm" style={{ borderRadius: '12px' }}>
                    <div className="card-body p-4">
                        <div className="d-flex justify-content-between align-items-center mb-4">
                            ...
                            {metas.length > 0 && (
                                <Link to="/metas" className="btn btn-sm btn-outline-primary">
                                    Ver Todas
                                </Link>
                            )}
                        </div>
                        {metas.length === 0 ? (
                            <div className="alert alert-info mb-0">
                                ...
                                <Link to="/metas/novo" className="alert-link ms-2">Criar uma agora</Link>
                            </div>
                        ) : (
                            <>
                                <ul className="list-unstyled mb-0">
                                    {metas.slice(0, 3).map((meta) => {
                                        ...
                                        const progress = meta.desiredAmount > 0 ? Math.min((currentValue / meta.desiredAmount) * 100, 100) : 0;
                                        ...
                                        return (
                                            <li key={meta.id} className="d-flex align-items-center mb-4 pb-3" style={{ borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
                                                <div className="me-3">
                                                    <i className={`bi ${progress >= 100 ? 'bi-check-circle-fill text-success' : 'bi-bullseye text-primary'} fs-4`}></i>
                                                </div>
                                                <div className="flex-grow-1">
                                                    <h6 className="mb-2 text-light fw-bold">{meta.name}</h6>
                                                    <div className="progress mb-2" style={{ height: '10px', backgroundColor: 'var(--color-primary-dark)', borderRadius: '5px' }}>
                                                        <div 
                                                    ...
                                                    <div className="d-flex justify-content-between align-items-center">
                                                        <small className="text-muted">
                                                            {formatCurrency(currentValue)} de {formatCurrency(meta.desiredAmount)}
                                                        </small>
                                                        <span className={`badge ${progress >= 100 ? 'bg-success' : 'bg-secondary'}`}>
                                                            {progress.toFixed(1)}%
                                                        </span>
                                                    </div>
                                                </div>
            ...
            <div className="card shadow-sm p-4">
              ...
              <div className="d-flex justify-content-between align-items-center mb-3">
                <h5 className="mb-0 text-light">
                  <i className="bi bi-graph-up me-2" style={{ color: '#7c3aed' }}></i> Evolução Financeira
                </h5>
                <div className="d-flex gap-2 align-items-center">
                  ...
                  <button
                    ...
                    style={{ 
                      backgroundColor: filterByDateRange.start === new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString().split('T')[0] ? '#7c3aed' : '#2d2d2d',
                      color: '#fff',
                  ...
                  <button
                    className="btn btn-sm"
                    style={{ 
                      backgroundColor: filterByDateRange.start === new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0] ? '#7c3aed' : '#2d2d2d',
                      color: '#fff',
                      border: '1px solid #3d3d3d',
                      fontSize: '12px',
                      padding: '4px 12px'
                  ...
                  <button
                    className="btn btn-sm"
                    style={{ 
                      backgroundColor: filterByDateRange.start === new Date(Date.now() - 90 * 24 * 60 * 60 * 1000).toISOString().split('T')[0] ? '#7c3aed' : '#2d2d2d',
                      color: '#fff',
                      border: '1px solid #3d3d3d',
                      fontSize: '12px',
                      padding: '4px 12px'
              ...
              <div className="mb-3">
                <div className="d-flex gap-2 flex-wrap align-items-center">
                  ...
                  <button
                    className={`btn btn-sm ${!filterByCategory ? 'active' : ''}`}
                    style={{ 
                      backgroundColor: !filterByCategory ? '#7c3aed' : '#2d2d2d',
                      color: '#fff',
                      border: '1px solid #3d3d3d',
                      fontSize: '11px',
                      padding: '2px 10px',
                  ...
                  {(() => {
                    ...
                    return Array.from(allCategories).sort().map(cat => (
                      <button
                        key={cat}
                        className={`btn btn-sm ${filterByCategory === cat ? 'active' : ''}`}
                        style={{ 
                          backgroundColor: filterByCategory === cat ? '#7c3aed' : '#2d2d2d',
                          color: '#fff',
                          border: '1px solid #3d3d3d',
                          fontSize: '11px',
                          padding: '2px 10px',
                        ...
                        onClick={() => setFilterByCategory(filterByCategory === cat ? '' : cat)}
                      >
                        {cat}
                      </button>
                    ));
              ...
              {timeSeriesData.length === 0 ? (
                ...
                <>
                  ...
                  <div className="chart-container" style={{ height: '350px', position: 'relative', borderRadius: '4px', padding: '20px' }}>
                    <svg width="100%" height="100%" viewBox="0 0 1000 320" style={{ overflow: 'visible' }}>
                      {(() => {
                        const filteredData = timeSeriesData.filter(d => 
                          d.date >= filterByDateRange.start && d.date <= filterByDateRange.end
                        );
                        if (filteredData.length === 0) {
                          return (
                            <text x="500" y="160" textAnchor="middle" fill="#888" fontSize="14">
                              Nenhum dado disponível para o período selecionado
                            </text>
                          );
                        }
                        ...
                        const maxValue = Math.max(
                          ...filteredData.map(d => Math.max(d.expenses, d.incomes)),
                          1
                        );
                        const minBalance = Math.min(...filteredData.map(d => d.balance), 0);
                        const maxBalance = Math.max(...filteredData.map(d => d.balance), 0);
                        const balanceRange = Math.max(Math.abs(maxBalance), Math.abs(minBalance)) || 1;
                        
                        const padding = 80;
                        const chartWidth = 900;
                        const chartHeight = 220;
                        const barWidth = Math.max(8, Math.min(20, (chartWidth / filteredData.length) * 0.7));
                        const barSpacing = chartWidth / (filteredData.length || 1);
                        ...
                        return (
                          <>
                            ...
                            {/* Eixos */}
                            <line x1={padding} y1={baseY} x2={padding + chartWidth} y2={baseY} stroke="#444" strokeWidth="1.5" />
                            <line x1={padding} y1={padding} x2={padding} y2={baseY} stroke="#444" strokeWidth="1.5" />
                            ...
                            {filteredData.map((d, i) => {
                              const x = padding + i * barSpacing + (barSpacing - barWidth * 2 - 4) / 2;
                              
                              // Barra de receitas
                              const incomeHeight = (d.incomes / maxValue) * chartHeight;
                              ...
                              return (
                                <g key={`bars-${i}`}>
                                  ...
                                  >
                                    <title>{`Receitas: ${formatCurrency(d.incomes)}\n${new Date(d.date).toLocaleDateString('pt-BR')}`}</title>
                                  </rect>
                            ...
                            <path
                              d={filteredData
                                .map((d, i) => {
                                  ...
                                  const normalizedBalance = (d.balance + Math.abs(minBalance)) / balanceRange;
                                  const y = baseY - normalizedBalance * chartHeight;
                                  return `${i === 0 ? 'M' : 'L'} ${x} ${y}`;
                                })
                                .join(' ')}
                              fill="none"
                            ...
                            {filteredData.map((d, i) => {
                              const x = padding + i * barSpacing + barSpacing / 2;
                              const shouldShow = i % Math.max(1, Math.floor(filteredData.length / 10)) === 0 || i === filteredData.length - 1;
                  ...
                  <div className="d-flex gap-4 justify-content-start mt-3">
                    <div className="d-flex align-items-center">
                      <div style={{ width: '12px', height: '12px', background: '#22c55e', marginRight: '6px', borderRadius: '2px' }}></div>
                      <small className="text-muted" style={{ fontSize: '12px' }}>Receitas</small>
                    </div>
                    <div className="d-flex align-items-center">
                      <div style={{ width: '12px', height: '12px', background: '#ef4444', marginRight: '6px', borderRadius: '2px' }}></div>
                      <small className="text-muted" style={{ fontSize: '12px' }}>Despesas</small>
                    </div>
                    <div className="d-flex align-items-center">
                      <div style={{ width: '20px', height: '2px', background: '#3b82f6', borderTop: '2px dashed #3b82f6', marginRight: '6px' }}></div>
                      <small className="text-muted" style={{ fontSize: '12px' }}>Saldo Acumulado</small>
                    </div>
                  </div>
                </>
              )}
          ...
          <div className="col-12">
            <div className="card shadow-sm p-4">
              <h5 className="mb-4 text-light">
                <i className="bi bi-tags-fill me-2 text-danger"></i> Despesas por Categoria
              </h5>
                ...
                <div className="row g-3">
                  {Object.entries(expensesByCategory)
                    ...
                    .map(([category, amount]) => {
                      ...
                      const percentage = totalDespesas > 0 ? ((amount / totalDespesas) * 100).toFixed(1) : '0.0';
                      return (
                        <div key={category} className="col-md-6 col-lg-4">
                          <div className="card border-danger h-100">
                            <div className="card-body">
                              <h6 className="card-title text-danger mb-2">{category}</h6>
        ...
        <div className="row g-4 mb-5">
          <div className="col-12">
            <div className="card shadow-sm p-4">
              <h5 className="mb-4 text-light">
                <i className="bi bi-tags-fill me-2 text-success"></i> Receitas por Categoria
              </h5>
              {Object.keys(incomesByCategory).length === 0 ? (
                ...
                <div className="row g-3">
                  {Object.entries(incomesByCategory)
                    .sort((a, b) => b[1] - a[1])
                    .map(([category, amount]) => {
                      const totalReceitas = Object.values(incomesByCategory).reduce((sum, val) => sum + val, 0);
                      const percentage = totalReceitas > 0 ? ((amount / totalReceitas) * 100).toFixed(1) : '0.0';
                      return (
                        <div key={category} className="col-md-6 col-lg-4">
                          <div className="card border-success h-100">
                            <div className="card-body">
                              <h6 className="card-title text-success mb-2">{category}</h6>
        ...
        <div className="mt-5">
          <h3 className="h4 border-bottom pb-2 mb-4 text-light">Informações da Organização</h3>
          <div className="p-4 rounded shadow-sm card">
            <p className="mb-1 text-light">
              <strong>{currentOrganization.name}</strong>
            </p>
            <p className="mb-0">
              <span className={`badge ${currentOrganization.isActive ? 'bg-success' : 'bg-secondary'}`}>
                {currentOrganization.isActive ? 'Status: Ativa' : 'Status: Inativa'}
              </span>
            </p>
          </div>
        </div>
